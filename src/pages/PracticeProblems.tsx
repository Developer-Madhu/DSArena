...
//   const { lives, noLives, isLoading: livesLoading, penalize, timeUntilRestore, formattedTimeRemaining } = useLivesManager(user?.id);
...
//         const cloudDraft = await loadDraft(user.id, problem.id);
//         if (cloudDraft) {
//           setCode(cloudDraft);
...
//       const result = await saveToCloud(user.id, problem.id, code);
//       if (result.success) {
...
//           saveToCloud(user.id, problem.id, newCode);
//         });
//       }
//     }, 2000);
//   };
...
// }
// Practice Problems Page with Problem-Solving Interface
// Complete implementation with proper save/retrieve functionality
import { useState, useEffect, useCallback, useRef } from 'react';
import { useNavigate, useParams } from 'react-router-dom';
import { Navbar } from '@/components/layout/Navbar';
import { CodeEditor } from '@/components/editor/CodeEditor';
...
export default function PracticeProblemsPage() {
  ...
  // Generate storage key for local/cloud storage
  const getStorageKey = useCallback((problemId: string, lang: string, userId?: string) => {
    return `practice-problem-draft-${userId || 'anonymous'}-${problemId}-${lang}`;
  }, []);

  const getCloudStorageKey = useCallback((problemId: string, lang: string) => {
  ...
  // Load saved code from storage (local or cloud)
  const loadSavedCode = useCallback(async (problemId: string, lang: string): Promise<string | null> => {
    // 1. Try local storage first (fastest)
    const localKey = getStorageKey(problemId, lang, user?.id);
    const savedLocalDraft = localStorage.getItem(localKey);
    if (savedLocalDraft) {
      console.log('✅ Loaded from local storage:', localKey);
      return savedLocalDraft;
    }

    // 2. Try cloud storage if user is logged in
    if (user?.id) {
      try {
        const { loadDraft } = await import('@/lib/progressStorage');
        const cloudKey = getCloudStorageKey(problemId, lang);
        const cloudDraft = await loadDraft(user.id, cloudKey);
        
        if (cloudDraft) {
          console.log('✅ Loaded from cloud storage:', cloudKey);
          // Cache to local storage for faster future loads
          localStorage.setItem(localKey, cloudDraft);
          return cloudDraft;
        }
      } catch (err) {
    ...
    return null;
  }, [user?.id, getStorageKey, getCloudStorageKey]);

  // Save code to storage (local and cloud)
  const saveCodeToStorage = useCallback(async (
    problemId: string, 
    lang: string, 
    codeToSave: string,
    ...
    localStorage.setItem(localKey, codeToSave);
    console.log('💾 Saved to local storage:', localKey);
    ...
    if (user?.id) {
      try {
        const { saveDraft: saveToCloud } = await import('@/lib/progressStorage');
        const cloudKey = getCloudStorageKey(problemId, lang);
        const result = await saveToCloud(user.id, cloudKey, codeToSave);
        
        if (result.success) {
          console.log('☁️ Saved to cloud storage:', cloudKey);
          if (showToast) {
            toast.success('Logic Uploaded to Mainframe');
          }
          return true;
  ...
  useEffect(() => {
    const initializeCode = async () => {
      ...
      console.log('🔄 Initializing code for:', problem.id, selectedLanguage);

      // Try to load saved code
      const savedCode = await loadSavedCode(problem.id, selectedLanguage);
      ...
      if (savedCode) {
        ...
        console.log('📝 Using starter code for:', selectedLanguage);
        setCode(starterCode);
      }
    };

    initializeCode();
  }, [problem?.id, selectedLanguage, loadSavedCode, getStarterCodeForLanguage]);
  ...
  const saveDraft = useCallback(async () => {
    ...
    const success = await saveCodeToStorage(problem.id, selectedLanguage, code, true);
  ...
  }, [problem, selectedLanguage, code, saveCodeToStorage]);
  ...
  const handleCodeChange = (newCode: string) => {
    ...
    saveTimeoutRef.current = setTimeout(async () => {
      if (problem) {
        ...
        await saveCodeToStorage(problem.id, selectedLanguage, newCode, false);
        setSavingStatus('saved');
        setTimeout(() => setSavingStatus('idle'), 2000);
      }
    }, 2000);
  ...
  // Load problem from URL or default
  const loadProblem = useCallback(async () => {
    try {
      setIsLoading(true);
      setError(null);
            ...
            <ResizablePanelGroup direction="vertical">
              ...
              <ResizablePanel defaultSize={60} minSize={30}>
                <div className="flex h-full flex-col">
                  ...
                  <div className="flex items-center justify-between border-t border-border px-4 py-3">
                    <span className="text-xs text-muted-foreground">
                      {solved ? "Problem completed" : "Auto-save enabled"}
                    </span>
                    <div className="flex gap-2">
                      {solved ? (
                        <div className="flex items-center gap-2 text-sm text-success">
